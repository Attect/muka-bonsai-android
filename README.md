...
- **本地推理**：基于 llama.cpp，支持 Bonsai 1-bit 27B（Q1_0）、Ternary 1.58-bit 27B（Q2_0 g128），以及 **Bonsai 2 27B**：PQ2_0（2.13 bpw，OpenCL GPU 加速）与 PTQ1_0（1.75 bpw，暂仅 CPU）
...
- **推理参数**：按模型独立保存配置档案，新模型首次加载时以该模型 GGUF 内的推荐采样参数为初值；上下文长度（最高支持模型上限）、最大输出（64–32768）、线程数、温度、top-k/top-p、KV 缓存量化（F16/Q8_0/Q4_0）
...
- Bonsai 2 量化类型：`PQ2_0`（与 `block_q2_0` 逐字节同构，OpenCL 复用同一批内核）与 `PTQ1_0`（28 字节/块的三值 trit 编码，目前仅 CPU）
